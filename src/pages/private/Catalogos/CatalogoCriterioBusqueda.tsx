      ...
      reader.onload = (e) => {
        ...
        const formattedData: any[] = jsonData.slice(1).map((row) => {
          ...
          properties.forEach((property, index) => {
            ...
            if (property === "Tipo de validación") {
              const id = tiposValidacion.filter(
                (x: any) => x.validacion === value
              )[0];

              obj.validacion = value;
              obj.idTipoValidacion =
                id !== undefined ? id.idTipoValidacion : undefined;
            }
        ...
        formattedData.forEach((item: any) => {
          ...
          if (!item.criterioBusqueda) {
            ...
            setMensajeRespuesta({
              indicador: 1,
              mensaje: 'No se encontró la columna "Criterio búsqueda"',
            });
            return;
          }
          if (!item.validacion) {
            ...
            setMensajeRespuesta({
              indicador: 1,
              mensaje: 'No se encontró la columna "Tipo de validación"',
            });
            return;
          }
          if (!item.valorExterno) {
            ...
            setMensajeRespuesta({
              indicador: 1,
              mensaje: 'No se encontró la columna "Valor externo"',
            });
            return;
          }
          ...
          if (
            typeof item.criterioBusqueda !== "string" ||
            item.criterioBusqueda === null
          )
            errores.push("Criterio búsqueda");
          if (item.criterioBusqueda.length > 50)
            errores.push("Criterio búsqueda (máximo 50 caracteres)");
          if (typeof item.validacion !== "string" || item.validacion === null)
            errores.push("Tipo de validación");
          if (item.validacion.length > 50)
            errores.push("Tipo de validación (mayor a 50 caracteres)");
          if (typeof valorExterno !== "string" || valorExterno === null)
            errores.push("Valor externo");
          if (item.valorExterno.length > 100)
            errores.push("Valor externo (mayor a 100 caracteres)");

          // Se identifican los nombres de criterios ya existentes
          if (
            criterios.filter(
              (x: any) => x.criterioBusqueda === item.criterioBusqueda
            ).length > 0
          ) {
            nombresRep +=
              nombresRep === ""
        ...
        if (tiposValidacionNa.length > 0) {
          ...
          setMensajeRespuesta({
            indicador: 2,
            mensaje:
              "No se encontraron los siguientes tipos de validación en el sistema: " +
              tiposValidacionNa +
        ...
        // Indicador de nombres repetidos
          ...
          setMensajeRespuesta({
            indicador: 2,
            mensaje:
              "Los siguientes criterios de búsqueda, ya existen en el sistema: " +
              nombresRep +
              ". Por lo que no serán cargados",
          });
          setShowSpinner(false);
        ...
        // Indicador de nombres repetidos en el archivo de carga
          ...
          setMensajeRespuesta({
            indicador: 2,
            mensaje:
              "Se encontraron registros con el mismo criterio de búsqueda, por lo que serán descartados para la carga. Favor validar los registros",
          });
          setShowSpinner(false);
        ...
        setListaImportar(
          formattedData.filter(
            (x: any) => x.idTipoValidacion && x.criterioBusqueda
          )
        );
            ...
            <Col md={6}>
              <Form.Group>
                ...
                <Select
                  ...
                  onChange={(e: any) => (
                    setIdTipoValidacion(e.value),
                    setTipoValidacionTexto(
                      tiposValidacion.filter(
                        (x: any) => x.idTipoValidacion === e.value
                      )[0].validacion
                    )
                  )}
